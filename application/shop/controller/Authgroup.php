...
use app\common\model\ShopAuthMenu;
use app\common\model\ShopAuthRule as AuthRule;
use app\common\model\ShopAuthGroup as AuthGroupModel;
...
//pj($map);
    ...
    public function del()
    {
        ...
        $res = model('ShopAuthGroup')->del($id);
        if ( $res ) {
    ...
    public function access(){
        ...
        // 4. 获取权限列表
        $nodes = ShopAuthMenu::order('sort', 'asc') -> select();
    ...
    public function doAccess()
    {
        if ( request()->isAjax() ) {
            ...
            if ( isset($rules) ) {
                ...
                $res = model("ShopAuthGroup")->updateById($id,['rules' => $datas]);
                if ( true == $res ) {
