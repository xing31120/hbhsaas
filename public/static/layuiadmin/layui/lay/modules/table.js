...
layui.define(["laytpl", "laypage", "layer", "form", "util"], function(e) {
	...
	F.prototype.config = {
			...
			if (a.elem = t(a.elem), a.where = a.where || {}, a.id = a.id || a.elem.attr("id") || e.index, a.request = t.extend({
					...
					e.page = a.page.curr = a.page.curr || 1, delete a.page.elem, delete a.page.jump), !a.elem[0]) {
				return e
			}
			...
			var e = this;
			e.fullSize(), e.setColsWidth(), e.scrollPatch(), e.calcFixedHeight()
		...
		}, F.prototype.calcFixedHeight = function() {
			var $ = layui.$;
			...
			if (typeof tableFlag !== "undefined" && tableFlag != "") {
				var deploy = layui.data("tableHeader")[tableFlag];
				if (deploy != null) {
					var arr = new Boolean(deploy) ? eval(deploy) : [];
					t.each(arr, function(index, ele) {
						...
						s({
							list: function() {
								...
								n.on("checkbox(LAY_TABLE_TOOL_COLS)", function(e) {
									if (typeof tableFlag !== "undefined" && tableFlag != "") {
										var tableHidden = "";
										t(".layui-unselect.layui-form-checkbox").each(function(index, ele) {
											var key = t(ele).siblings("input").attr("data-key");
											var parentkey = t(ele).siblings("input").attr("data-parentkey");
											var checked = t(ele).attr("class").indexOf("layui-form-checked") !== -1 ? true : false;
											tableHidden += "{'key':'" + key + "','check':" + checked + ",'parentKey':" + (parentkey ?
												parentkey : "''") + "},"
										});
										layui.data("tableHeader", {
											key: tableFlag,
											value: "[" + tableHidden + "]"
										})
									}
