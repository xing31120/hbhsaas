...
function listObject($ossDir,$op) {
    $isexist = $this->ossobj->is_object_exist($this->ossbuket, $ossDir);
    if($isexist){
        $options['max-keys'] = $op['limit'];//每页显示的数量
        $options['marker'] = $op['p'];
        $list = $this->ossobj->list_object($this->ossbuket, $options);
        return $list;
    }else{
        return [];
    }
