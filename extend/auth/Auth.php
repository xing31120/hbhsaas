...
class Auth
{
    ...
    protected $config = [
        ...
        'auth_group_access' => 'shop_auth_group_access', // 用户-用户组关系表
        'auth_rule' => 'shop_auth_rule', // 权限规则表
        'auth_user' => 'shop_user', // 用户信息表
    ];
    ...
    public function getGroups($uid)
    {
        ...
        // 执行查询
        $user_groups = Db::view($auth_group_access, 'user_id,role_id')
            ->view($auth_group, 'name,rules', "{$auth_group_access}.role_id={$auth_group}.id", 'LEFT')
            ->where("{$auth_group_access}.user_id='{$uid}' and {$auth_group}.status='1'")
            ->select();
